...
Every MAC is a switch with `eth_ports(mac)` front ports; a MAC wired to one PHY is the one-port
case. The port is the handle: `EthPort {mac, port}` opens, sends, receives, reports link and counts
drops, and callbacks carry the context it was opened with. The tag format and the chip's register
map never leave urt.
...
- MACs are named in the vendor's space by the platform driver (`ge1`, `ge2` on MT7621); OpenWatt
  matches on the name.
- `eth_ports(mac)`: front ports of a MAC, 1 or more; the frontend refuses a port already open.
- RX: frames arrive on the port's callback; urt strips the tag and attributes the source port.
- TX: `eth_tx` sends on the port; urt builds the tag.
- Link events arrive on the port's callback; the MAC comes up with its first open port and goes
  down with its last.
...
- One class, `/interface/ethernet`, with identity properties:
  - `device`: the MAC, in vendor naming (`ge1`); unset is the first MAC.
  - `port`: the front port, in the chip's numbering; default 0.
- `validate()` rejects an unknown `device` or a `port` the MAC does not have; a second interface
  on an open port fails at startup, since urt refuses the open.
- Unclaimed front ports are off: isolated, PHY powered down.
- `wire_send` sends on the port; RX and link events come back through the port's callback with
  the interface as context, so no lookup table exists in OpenWatt.
- Each port has its own address, supplied by urt from the board (hEX S: hard_config base + index,
  ether1..5 = +0..+4, sfp1 = +5).
...
1. urt port API and MT7530 driver; standalone ports; per-port link from the port status
