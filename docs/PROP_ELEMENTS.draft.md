...
compile-time literal. The typed accessor reads that element's `Scalar` register (or text slot)
reinterpreted through the declared type; the synthesized setter writes through the normal element
write path. Prop index == element index, so `eid` is `_id.element(prop_index!(T, "baud") + 1)`
...
format's `Constraint`), optional `Check!` function, optional `OnChange!` member function, and the
`Category!`/`PropFlags!` presentation metadata the legacy form takes positionally.
...
Typed accessors are hand-written one-liners over `prop_value`/`prop_write` rather than generated
by a mixin, so internal call sites keep reading `baud()` and assigning `baud = x`. A mixin would
remove that boilerplate but has to invent the accessor names and their const-ness from the
declaration; three lines per property is a fair price for not doing that yet. Both spellings
reach the same synthesized path the console and sync use, so there is exactly one write path.

### Validation: two layers, both before commit

The synthesized setter runs, in order:

1. **Variant -> T conversion.** Every writer passes through it, including typed internal writes:
   `prop_write` boxes and the setter unboxes, so one path validates rather than two. That costs a
   round trip per internal write, and buys a single place where range and type refusals are
   phrased.
2. **Per-prop check function**: `const(char)[] function(ref T value)` -- static, typed, may
   normalise (rewrite the value through the ref) or reject (return the error). It runs on the
   typed value, so it sees `Parity.mark`, not a Variant. Instance-dependent validation does not
   belong here; that is `validate()`'s job, unchanged.
3. **Universal constraint**: `format.constraint.check(v)` inside `Element.try_set` -- min/max
   from the declaration, enforced in the data model for every writer (console, sync `set`,
   automation, internal code). This is the same enforcement point the element write path needs
   anyway for remote `set` on ordinary elements; property projection inherits it. Constraint also
   rides the `type` frame as UI/schema metadata, so remote UIs can range-check before sending.
