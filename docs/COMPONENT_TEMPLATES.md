...
- `circuit: string` - Live or static circuit name for this port. Site config
  ...
  car circuit. These reported identities are read-only; appliance port bindings
  override them without editing the element. The Tesla TWC exposes `status.vin`
  as the reported identity and aliases `car.circuit` to it.
...
- `charge: Port` - The charge terminal, `role=connection`, `flow=consume`.
  ...
  no configuration.
- `charge.meter: EnergyMeter` (type: `single-phase` or `three-phase` for typical
...
- `charge.control: PowerControl` - Energy-app charge-rate actuator. Charge-only vehicles:
