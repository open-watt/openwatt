...
Members wait offline while their master is unavailable; a bridge keeps running
when a member is unavailable. See the bridge port collection below.
...
The member carries `S` even when the master is absent or disabled, and can run
only while the master is online. It never delivers standalone traffic while
waiting for its master. A bridge remains running with unavailable members,
...
Rows with either endpoint dynamic are also dynamic and excluded from configuration
export. Rows with static endpoints persist. The existing `add bridge=... interface=...`
form still works; omitting `name` generates a membership name.
