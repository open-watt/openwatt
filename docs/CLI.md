...
| `type` | Node type, from the device's own capability report. |
| `state` | `ready` once interviewed, `scanning` while an attempt is in flight, `retry <n>s` while backing off, `pending` when due, `offline` when detached. |
...
A node that has been heard from but cannot yet be named is listed separately as
`unresolved`, with `probing` while an address request is outstanding.
