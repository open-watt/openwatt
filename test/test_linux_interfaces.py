...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ow503-") as temporary:
        ...
        try:
            ...
            assert get(can_path, left, "running") == "true"
            print("PASS membership persistence, independent link state, reassignment and validation", flush=True)
            ...
            with socket.socket(socket.AF_CAN, socket.SOCK_RAW, socket.CAN_RAW) as tx, socket.socket(socket.AF_CAN, socket.SOCK_RAW, socket.CAN_RAW) as rx:
                ...
                downs = get(can_path, left, "link-downs")
                command(f"{bridges}/set canbridge disabled=true")
                assert get(can_path, left, "running") == "true"
                drops = int(get(can_path, left, "rx-dropped"))
                tx.send(struct.pack("=IB3x8s", 0x123, 3, b"abc"))
                rx.settimeout(0.3)
                try:
                    rx.recv(16)
                    raise AssertionError("Offline bridge forwarded a frame")
                except socket.timeout:
                    pass
                assert int(get(can_path, left, "rx-packets")) == 4
                assert int(get(can_path, left, "rx-dropped")) == drops + 1
                command(f"{bridges}/set canbridge disabled=false")
                tx.send(struct.pack("=IB3x8s", 0x123, 3, b"abc"))
                rx.settimeout(2)
                assert struct.unpack("=IB3x8s", rx.recv(16))[:2] == (0x123, 3)
                assert get(can_path, left, "link-downs") == downs
            ...
            wire2_index = Path("/sys/class/net/ow503eth2/ifindex").read_text().strip()

            def wire2_has_socket():
                return any(row.split()[4] == wire2_index for row in Path("/proc/net/packet").read_text().splitlines()[1:])

            assert get(ether_path, "wire2", "running") == "false"
            assert wire2_has_socket(), "Pending startup must have opened the interface socket"
            command("/interface/bridge/set ethbridge disabled=true")
            assert wire2_has_socket(), "Master loss must not interrupt member startup"
            assert get(ether_path, "wire2", "running") == "false"
            ip("link", "set", "ow503peer2", "up")
            time.sleep(2)
            assert get(ether_path, "wire2", "running") == "true"
            assert get(bridges, "ethbridge", "running") == "false"
            command("/interface/bridge/set ethbridge disabled=false")
            print("PASS member startup independently of master state", flush=True)
            ...
            downs = get(ether_path, "wire", "link-downs")
            command(f"{bridges}/set ethbridge disabled=true")
            assert get(ether_path, "wire", "running") == "true"
            assert not Path("/sys/class/net/ow503eth/master").exists()
            drops = int(get(ether_path, "wire", "rx-dropped"))
            with socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x88B6)) as tx, socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x88B6)) as rx:
                tx.bind(("ow503peer", 0))
                rx.bind(("ow503peer2", 0))
                rx.settimeout(0.3)
                ...
                try:
                    rx.recv(2048)
                    raise AssertionError("Offline kernel bridge forwarded a frame")
                except socket.timeout:
                    pass
            before = int(get(ether_path, "wire", "rx-packets"))
            assert int(get(ether_path, "wire", "rx-dropped")) == drops + 1
            assert get(ether_path, "wire", "link-downs") == downs
            command(f"{bridges}/set ethbridge disabled=false")
            assert Path("/sys/class/net/ow503eth/master").exists()
            ...
            ip("link", "add", "ow503eth3", "type", "veth", "peer", "name", "ow503peer3")
            ip("link", "set", "ow503eth3", "up")
            ip("link", "set", "ow503peer3", "up")
            command(f"{ether_path}/add name=wire3 adapter=ow503eth3")
            command("/interface/bridge/port/add name=eth-third bridge=ethbridge interface=wire3")
            with socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x88B6)) as tx:
                tx.bind(("ow503peer", 0))
                tx.send(frame)
            time.sleep(0.3)
            command("/interface/bridge/set ethbridge vlan-filtering=true", delay=1)
            assert not Path("/sys/class/net/ow503eth/master").exists()
            unicast = bytes.fromhex("02000000050302000000050688b6") + b"after-offload" * 4
            with socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x88B6)) as tx, socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x88B6)) as rx:
                tx.bind(("ow503peer3", 0))
                rx.bind(("ow503peer", 0))
                rx.settimeout(2)
                tx.send(unicast)
                assert rx.recv(2048) == unicast, "CPU-port learning survived the offload-to-software transition"
            command("/interface/bridge/set ethbridge vlan-filtering=false")
            command("/interface/bridge/port/remove eth-third")
            command(f"{ether_path}/remove wire3")
            ip("link", "del", "ow503eth3")
            print("PASS learned unicast forwarding after offload removal", flush=True)
