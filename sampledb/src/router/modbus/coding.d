...
struct ModbusRTUFrame
{
    ...
    ushort crc;
    align(4) ModbusData data;

    string toString() const
	{
//        string result = format("[%02x %02x %s %02x %02x]\n  Device: %x, Function: %02x", deviceAddress, functionCode, toHexString(data.raw), crc & 0xFF, crc >> 8, deviceAddress, functionCode);
        string result = format("Device: %x, Function: %02x", deviceAddress, functionCode);
        string value = data.toString(functionCode);
        if (value)
            result ~= "\n  " ~ value;
        return result;
    }
...
struct ModbusTCPFrame
{
    ...
    string toString() const
	{
        string result = format("[%s]\n  Transaction: %d, UnitId: %d, Function: %02x", toHexString(data.raw), transactionId, unitId, functionCode);
        return result;
    }
...
static assert(ModbusData.sizeof == 40);
static assert(ModbusRTUFrame.sizeof == 64);
static assert(ModbusTCPFrame.sizeof == 64);


ptrdiff_t checkCompleteModbusRTUFrame(const(ubyte)[] packet)
{
    // not sure if this is legit... there shouldn't be split packets in modbus RTU frames...
//    if (packet.length <= 2)
//        return 0;
	if (packet.length > 2)
	{
        ushort crc = packet[$-2 .. $][0..2].littleEndianToNative!ushort;
        if (calculateModbusCRC(packet[0 .. $-2]) == crc)
            return packet.length;
	}
	return -1;
}

ModbusRTUFrame decodeModbusRTUFrame(const(ubyte)[] packet, RequestType type)
{
    ModbusRTUFrame result;
    ...
    result.deviceAddress = packet[0];
    result.functionCode = cast(FunctionCode)packet[1];
    result.crc = packet[$-2 .. $][0..2].littleEndianToNative!ushort;

    assert(packet.length >= 4);

	if (calculateModbusCRC(packet[0 .. $-2]) != result.crc)
	{
        throw new Exception("Invalid packet: Bad CRC");
    }

    result.data = parseValues(type, result.functionCode, packet[2 .. $-2], result.smallStore);

    return result;
}

ptrdiff_t checkCompleteModbusTCPFrame(const(ubyte)[] packet)
{
    // we need enough data to attempt to decode a packet
    if (packet.length < 8)
        return 0;

    // confirm the protocol matches
	ushort protoId = packet[2..4].bigEndianToNative!ushort;
    if (protoId != 0)
        return -1;

	ushort length = packet[4..6].bigEndianToNative!ushort;
    if (packet.length >= length + 6)
        return length + 6;
	return 0;
}

ModbusTCPFrame decodeModbusTCPFrame(const(ubyte)[] packet, RequestType type)
{
    ModbusTCPFrame result;
    ...
    if (protoId != 0)
	{
        throw new Exception("Invalid packet: Invalid modbus TCP protocol id");
	}

    result.transactionId = packet[0..2].bigEndianToNative!ushort;
    ushort length = packet[4..6].bigEndianToNative!ushort;
    result.unitId = packet[6];
    result.functionCode = cast(FunctionCode)packet[7];

    assert(packet.length >= 8);

    result.data = parseValues(type, result.functionCode, packet[8 .. $], result.smallStore);
