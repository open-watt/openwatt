...
# Classify by section name, not nm's type letters: those cannot tell .data
# from .data.rel.ro, and conflating them reports flash as RAM. Anything
# holding a Type.init blob or a vtable lands in the latter and is flash on a
# target that executes in place.
#
# Matched by substring because targets qualify the names: ESP-IDF emits
# .dram0.bss and .dram0.data, and rodata has to be excluded explicitly or
# "data" swallows it.

def classify(section):
    """'data', 'bss', or None for anything that is not writable RAM."""
    s = section.lower()
    if "rodata" in s or "rel.ro" in s:
        return None
    if "bss" in s or "noinit" in s or "common" in s:
        return "bss"
    if "data" in s:
        return "data"
    return None
