...
{
    IPv6Addr source = stack.select_source_v6_on_iface(target, iface);
...
void on_router_advert(ref IPStack stack, ref const IPv6Header ip, const(ubyte)[] message, BaseInterface iface)
{
    ...
    ra_route(station, IPv6NetworkAddress(IPv6Addr.any, 0), router, loadBigEndian(cast(const(ushort)*)(message.ptr + 6)), now);
...
void slaac_update(ref IPStack stack, MonoTime now)
{
    ...
    for (size_t i = _slaac_prefixes.length; i > 0; --i)
    {
        ...
        if (IPv6Address address = prefix.address.get)
            address.deprecated_ = now >= prefix.preferred_until;
    }

    for (size_t i = _slaac_routes.length; i > 0; --i)
    {
        ref route = _slaac_routes[i - 1];
        BaseInterface iface = route.iface.get;
        if (!iface || (iface.flags & ObjectFlags.slave) || now >= route.expires)
            remove_slaac_route(i - 1);
    }
...
enum Duration valid_floor            = 7200.seconds;    // RFC 4862 5.5.3(e): unauthenticated RAs cannot cut below this
...
struct SlaacRoute
{
    IPv6NetworkAddress destination;
    IPv6Addr gateway;
    MonoTime expires;
    ObjectRef!BaseInterface iface;
...
__gshared Array!SlaacRoute  _slaac_routes;
...
void ra_route(EthernetStation iface, IPv6NetworkAddress destination, IPv6Addr gateway, uint lifetime, MonoTime now)
{
    foreach (i, ref state; _slaac_routes[])
    {
        if (state.destination != destination || state.gateway != gateway || state.iface.get !is iface)
            continue;
        if (lifetime == 0)
            remove_slaac_route(i);
        else
        {
            state.expires = now + lifetime.seconds;
            if (!state.route)
                state.route = create_slaac_route(iface, destination, gateway);
        }
        return;
    }
    if (lifetime == 0)
    ...
    if (!state.route)
    {
        log.error("failed to create dynamic route ", destination);
        return;
    }
    _slaac_routes ~= state;
    if (gateway == IPv6Addr.any)
        log.info("on-link ", destination, " on ", iface.name, " (lifetime ", lifetime, "s)");
    else
        log.info("default route via ", gateway, " on ", iface.name, " (lifetime ", lifetime, "s)");
}

IPv6Route create_slaac_route(EthernetStation iface, IPv6NetworkAddress destination, IPv6Addr gateway)
{
    const(char)[] name = Collection!IPv6Route().generate_name(tconcat(iface.name[], ".ra"));
    if (gateway == IPv6Addr.any)
        return Collection!IPv6Route().create(name, ObjectFlags.dynamic, NamedArgument("destination", destination), NamedArgument("out-interface", cast(BaseInterface)iface));
    return Collection!IPv6Route().create(name, ObjectFlags.dynamic, NamedArgument("destination", destination), NamedArgument("gateway", gateway), NamedArgument("out-interface", cast(BaseInterface)iface));
}

void remove_slaac_route(size_t index)
{
    if (IPv6Route route = _slaac_routes[index].route.get)
        route.destroy();
    _slaac_routes.removeSwapLast(index);
}

void ra_prefix(ref IPStack stack, EthernetStation iface, const(ubyte)[] option, MonoTime now)
{
    ubyte prefix_length = option[2];
    ubyte flags = option[3];
    uint valid = loadBigEndian(cast(const(uint)*)(option.ptr + 4));
    uint preferred = loadBigEndian(cast(const(uint)*)(option.ptr + 8));
    if (prefix_length > 128 || preferred > valid)
        return;
    IPv6NetworkAddress prefix = IPv6NetworkAddress(load_ipv6_address(option.ptr + 16), prefix_length);
    prefix.addr = prefix.get_network;
    if (prefix.addr.is_link_local || prefix.addr.is_multicast)
        return;
    if (valid > max_lifetime_s)
        valid = max_lifetime_s;
    if (preferred > max_lifetime_s)
        preferred = max_lifetime_s;
...
void ra_autonomous(ref IPStack stack, EthernetStation iface, IPv6NetworkAddress prefix, uint valid, uint preferred, MonoTime now)
{
    if (prefix.prefix_len != 64)
    {
        log.warning("ignoring RA prefix ", prefix, ": SLAAC needs a /64 for the interface identifier");
        return;
    }
    foreach (ref state; _slaac_prefixes[])
    {
        if (state.iface.get !is iface || state.formed.s[0 .. 4] != prefix.addr.s[0 .. 4])
            continue;
        Duration remaining = state.valid_until - now;
        if (valid.seconds > valid_floor || valid.seconds > remaining)
            state.valid_until = now + valid.seconds;
        else if (remaining > valid_floor)
            state.valid_until = now + valid_floor;
        state.preferred_until = now + preferred.seconds;
        return;
    }
    if (valid == 0)
        return;
...
void create_slaac_address(ref SlaacPrefix state)
{
    ...
    state.address = Collection!IPv6Address().create(name, ObjectFlags.dynamic, NamedArgument("address", IPv6NetworkAddress(state.formed, 128)), NamedArgument("interface", iface));
    if (!state.address)
    {
    ...
    log.info("SLAAC address ", state.formed, " on ", iface.name);
}
...
void retire_slaac(ref IPStack stack, EthernetStation iface)
{
    for (size_t i = _slaac_prefixes.length; i > 0; --i)
        if (_slaac_prefixes[i - 1].iface.get is iface)
            remove_slaac_prefix(stack, i - 1);
    for (size_t i = _slaac_routes.length; i > 0; --i)
        if (_slaac_routes[i - 1].iface.get is iface)
            remove_slaac_route(i - 1);
}
