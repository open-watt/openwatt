...
// A station binds one segment. Otherwise the ether stack applies the local address normally.
...
else version (Windows)
{
    ...
    sockaddr_in to_sockaddr_in(ref const InetAddress a) nothrow @nogc
    {
        ...
        (cast(ubyte*)&sa.sin_port)[0 .. 2] = nativeToBigEndian(a._a.ipv4.port);
    ...
    InetAddress from_sockaddr_in(ref const sockaddr_in sa) nothrow @nogc
    {
        ...
        const(ubyte)[2] port = (cast(const(ubyte)*)&sa.sin_port)[0 .. 2];
        return InetAddress(ip, port.bigEndianToNative!ushort);
    }
