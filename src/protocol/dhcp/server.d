...
private:
    ...
    void handle_discover(ref const DhcpHeader req, MACAddress client_mac, ref DhcpParse p)
    {
        ...
        else
        {
            ...
            {
                version (DebugDHCP)
                    log.debug_("DISCOVER from ", client_mac, " over mac-limit (", _mac_limit, ")");
                return;
            }
            ...
            {
                version (DebugDHCP)
                    log.warning("pool exhausted; cannot offer to ", client_mac);
                return;
            }
    ...
    void handle_request(ref const DhcpHeader req, MACAddress client_mac, ref DhcpParse p)
    {
        ...
        version (DebugDHCP)
            log.info("ACK ", target, " to ", client_mac, " lease=",
    ...
    {
        version (DebugDHCP)
            log.debug_("NAK to ", client_mac, ": ", reason);
