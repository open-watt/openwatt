...
private:
    ...
    void send_ia_message(Dhcp6MsgType type)
    {
        ...
        b.transmit(s, link_local_for(s.mac), dhcp6_multicast, ether_multicast_dhcp6,
    ...
    bool accept_bindings(ref Dhcp6Parse p, bool commit)
    {
        ...
        if (_request_address && p.ia(Dhcp6Option.ia_na, ia))
        {
            IaAddr a;
            Dhcp6Status status;
            if (Dhcp6Parse.status_of(ia.options, status) && status == Dhcp6Status.success && Dhcp6Parse.ia_addr(ia, a) && a.valid)
            {
                _addr = a.addr;
                got_any = true;
        ...
        if (_request_prefix && p.ia(Dhcp6Option.ia_pd, ia))
        {
            IaPrefix pf;
            Dhcp6Status status;
            if (Dhcp6Parse.status_of(ia.options, status) && status == Dhcp6Status.success && Dhcp6Parse.ia_prefix(ia, pf) && pf.valid)
            {
    ...
    void apply_lease()
    {
        ...
        if (_pd_prefix != IPv6Addr.any)
        {
            ...
            {
                pool.prefix = IPv6NetworkAddress(_pd_prefix, _pd_prefix_len);
            }
            else
            {
                ...
                _pd_pool = Collection!IPv6Pool().create(
                    pn,
                    ObjectFlags.dynamic,
                    NamedArgument("prefix", IPv6NetworkAddress(_pd_prefix, _pd_prefix_len)));
                if (!_pd_pool)
