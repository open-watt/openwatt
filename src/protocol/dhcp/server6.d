...
protected:
    ...
    override CompletionStatus startup()
    {
        ...
        foreach (l; Collection!DHCP6Lease().values)
        {
            ...
            if (!_pool.contains(lease.address))
                continue;
            if (lease.is_prefix)
                _pool.reserve_prefix(IPv6NetworkAddress(lease.address, lease.prefix_length));
            else
    ...
    override void update()
    {
        ...
        foreach (l; Collection!DHCP6Lease().values)
        {
            ...
            {
                if (lease.is_prefix)
                    _pool.release_prefix(IPv6NetworkAddress(lease.address, lease.prefix_length));
                else
...
private:
    ...
    void incoming_packet(ref const Packet pkt, BaseInterface, PacketDirection dir, void* user_data)
    {
        ...
        if (src_port != dhcp6_client_port || dst_port != dhcp6_server_port)
            return;
        ...
        if (client_duid.length == 0 || client_duid.length > max_duid_size)
            return;
                                ...
                                IPv6Addr client_ip, MACAddress client_mac, Dhcp6MsgType reply_type)
    {
        ...
        if (p.ia(Dhcp6Option.ia_pd, ia))
        {
            ...
            IPv6NetworkAddress prefix = lease ? IPv6NetworkAddress(lease.address, lease.prefix_length)
                            : _pool.allocate_prefix(IPv6NetworkAddress(has_hint ? requested.prefix : IPv6Addr.any, _delegation_length));

            size_t body_ = b.begin_ia(Dhcp6Option.ia_pd, ia.iaid, valid_s / 2, valid_s * 4 / 5);
            if (prefix.addr == IPv6Addr.any)
            {
            ...
            {
                if (!lease)
                    lease = make_lease(prefix.addr, prefix.prefix_len, duid_str, ia.iaid);
                if (lease)
                {
                    touch_lease(lease, hold);
                    b.add_ia_prefix(prefix.addr, prefix.prefix_len, valid_s, valid_s);
                    if (committing)
                        log.info("delegated ", prefix.addr, "/", prefix.prefix_len, " to ", duid_str, " iaid=", ia.iaid);
                }
                else
                {
        ...
        add_dns(b);
        b.transmit(station, _server_ll, client_ip, client_mac, dhcp6_server_port, dhcp6_client_port);
    }
