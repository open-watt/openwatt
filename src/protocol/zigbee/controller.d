...
private:
    ...
    void apply_zone_status(EUI64 eui, ubyte endpoint, ushort cluster, ushort zone_status)
    {
        ...
        {
            for (SampleElement* e = find_sample_element(eui, endpoint, cluster, bit_attrs[i]); e; e = e.next)
            ...
            foreach (ref NodeMap.Cluster c; ep.clusters.values)
            {
                ...
                foreach (ref NodeMap.Attribute a; c.attributes.values)
                {
                    if (a.last_updated == SysTime())
                        continue;
                    for (SampleElement* e = find_sample_element(node.eui, ep.endpoint, c.cluster_id, a.attribute_id); e; e = e.next)
    ...
    void prime_elements(NodeMap* node)
    {
        ...
        {
            SampleElement* probe = _sample_elements[probe_key];
            if (!chain_needs_prime_read(probe, *node))
                continue;
            ...
            foreach (ref other_key; _sample_elements.keys)
            {
                ...
                if (other_key[1] >= probe_key[1])
                    continue;
                if (!chain_needs_prime_read(_sample_elements[other_key], *node))
                    continue;
                first = false;
                break;
            }
            ...
            size_t n;
            foreach (SampleElement* e; _sample_elements.values)
            {
                if (e.endpoint != probe.endpoint || e.cluster != probe.cluster || !chain_needs_prime_read(e, *node))
                    continue;
            ...
            while (msg.length >= 3)
            {
                ...
                if (taken < 0)
                    break;
                for (SampleElement* e = find_sample_element(node.eui, probe.endpoint, probe.cluster, attr_id); e; e = e.next)
    ...
    bool chain_needs_prime_read(SampleElement* head, ref NodeMap node)
    {
        for (SampleElement* e = head; e; e = e.next)
            if (element_needs_prime_read(*e, node))
                return true;
        return false;
    }
    ...
    bool needs_priming(ref NodeMap node)
    {
        ...
        foreach (ref ep; node.endpoints.values)
            if (ep.has_ias_zone && !ep.ias_enrolled && ep.ias_attempts < 3)
                return true;
        foreach (SampleElement* e; _sample_elements.values)
            if (node.prime_attempts < 3 && chain_needs_prime_read(e, node))
                return true;
        return false;
    }
