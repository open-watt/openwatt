...
//        register_address_extractor(PacketType.zigbee_nwk, &extract_nwk_src_address, &extract_nwk_dst_address);
    ...
    FunctionCommandState zcl_read(Session session, ZigbeeEndpoint source, ref const(Variant) node, ubyte endpoint, ushort cluster, const(ushort)[] attributes)
    {
    ...
    static FunctionCommandState tuya_read(Session session, ZigbeeEndpoint source, NodeMap* nm, ubyte endpoint, const(ushort)[] attributes)
    {
