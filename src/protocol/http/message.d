...
bool body_included(ref const HTTPMessage message)
{
    if (message.method == HTTPMethod.HEAD || message.method == HTTPMethod.TRACE || message.method == HTTPMethod.CONNECT)
        return false;
    if (message.content.length == 0 && !(message.flags & HTTPFlags.ForceBody) && !(message.method == HTTPMethod.POST || message.method == HTTPMethod.PUT || message.method == HTTPMethod.PATCH))
        return false;
    return true;
}
