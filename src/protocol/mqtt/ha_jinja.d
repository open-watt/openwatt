...
unittest
{
    ...
    // a null default must compile+parse: jinja 'else none' emits a null literal
    String null_source;
    Expression* null_expression;
    assert(compile_jinja_template("{{ value | int / 10 if value | is_number else none }}", null_source, null_expression));
    scope(exit) null_expression.free_expression();
    assert(null_source[] == "$select($is_number($value), $to_int($value) / 10, null)");
}
