...
nothrow @nogc:
    ...
    bool sendRequest(ref const MACAddress server, ref const ModbusPDU request, ModbusResponseHandler response_handler, ModbusErrorHandler error_handler = null, ubyte num_retries = 0, ushort timeout = 500, PCP pcp = PCP.be, bool dei = false) nothrow @nogc
    {
        ...
        _pending.pushBack(PendingRequest(now, now, request, seq, num_retries, timeout, server, response_handler, error_handler, cast(ubyte) tag));
        return true;
    }
...
private:
    ...
    void incoming_packet(ref const Packet p, BaseInterface iface, PacketDirection dir, void* user_data) nothrow @nogc
    {
        ...
        else if (snoop_handler)
        {
            ...
            if (_pending[0].sequence_number != seq)
            {
                if (type != ModbusFrameType.request)
                    return;

                _pending[0].request_time = p.creation_time;
                _pending[0].request = ModbusPDU(cast(FunctionCode) message[4], message[5 .. $]);
                _pending[0].sequence_number = seq;
                _pending[0].server = p.eth.dst;
            }
            else
            {
                if (type != ModbusFrameType.response || _pending[0].request_time == SysTime())
                    return;

                ModbusPDU response = ModbusPDU(cast(FunctionCode) message[4], message[5 .. $]);
                snoop_handler(p.eth.src, _pending[0].request, response, _pending[0].request_time, p.creation_time);
            }
        }
    }
