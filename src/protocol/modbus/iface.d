...
nothrow @nogc:
    ...
    void protocol_changed()
    {
        ...
        if (protocol == ModbusProtocol.tcp && stream)
        {
            ...
            if (dyn_cast!SerialStream(stream))
    ...
    void stream_changed(ref const SampleUpdate update)
    {
        ...
        if (Stream s = stream)
        {
            ...
            static if (has_ip)
            {
                ...
                if (auto tcpStream = dyn_cast!TCPStream(s))
...
protected:
    ...
    override void online()
    {
        ...
        {
            import router.stream.serial : SerialStream;
            if (auto serial = dyn_cast!SerialStream(stream))
            {
...
private:
    ...
    void compute_timing(uint baud)
    {
        ...
        // inter-frame gap in microseconds: 3.5 x 11 x 1_000_000 / baud
        _gap_time_us = baud <= 19_200 ? 38_500_000 / baud : 1_750;

        // transport timeout: 2x max frame time + 200ms processing margin
        // max RTU frame = 256 bytes x 11 bits/char
