...
nothrow @nogc:
    ...
    override void init()
    {
        g_app.console.register_collection!SpinelClient();
    }

    override void update()
    {
        Collection!SpinelClient().update_all();
    }
}
...
// The spinel format string for a command's argument payload (the header and command id are added by the
// caller). Kept as a CTFE switch rather than an enum AA so it can drive SpinelTypeTuple!(...) at instantiation.
string command_format(Command cmd) pure nothrow @nogc
{
    switch (cmd)
    {
        case Command.PROP_VALUE_GET:
            return "i";
        case Command.PROP_VALUE_SET:
        case Command.PROP_VALUE_INSERT:
        case Command.PROP_VALUE_REMOVE:
        ...
        case Command.PEEK:
            return "LS";
        case Command.PEEK_RET:
        case Command.POKE:
            return "LSD";
        case Command.PROP_VALUE_MULTI_GET:
            return "A(i)";
        case Command.PROP_VALUE_MULTI_SET:
        case Command.PROP_VALUES_ARE:
            return "A(t(iD))";
        default:
            return "";
    }
...
// SPINEL property keys (core subset; see draft-rquattle-spinel-unified section 5)
enum SpinelProperty : uint
{
    LAST_STATUS = 0,
    PROTOCOL_VERSION = 1,
    NCP_VERSION = 2,
