...
private:
    ...
    void subscribe_vehicle_controls()
    {
        ...
        _charging_amps = vehicle.find_element("connection.control.setpoint");
    ...
    void publish_charge_state(ref const TeslaChargeState cs)
    {
        ...
        if (cs.charger_voltage.present)
            v.write_element("connection.meter.voltage", Quantity!(int, ScaledUnits.volt)(cs.charger_voltage.value), now);
        if (cs.charger_actual_current.present)
            v.write_element("connection.meter.current", Quantity!(int, ScaledUnits.ampere)(cs.charger_actual_current.value), now);
        if (cs.charger_power.present)
            v.write_element("connection.meter.power", Quantity!(int, ScaledUnits.watt)(cs.charger_power.value * 1000), now);
        if (cs.charge_energy_added.present)
            v.write_element("connection.meter.import", Quantity!(float, KilowattHour)(cs.charge_energy_added.value), now);

        if (cs.charge_current_request_max.present)
            v.write_element("connection.control.max", Quantity!(int, ScaledUnits.ampere)(cs.charge_current_request_max.value), now);
        if (cs.charging_amps.present)
            v.write_element("connection.control.setpoint", Quantity!(int, ScaledUnits.ampere)(cs.charging_amps.value), now, &vehicle_control_change);
