...
protected:
    ...
    override bool materialise()
    {
        ...
        builder.constant(info, "type", "power-regulator");

        Component status = builder.component("status", "DeviceStatus");
        _online = add_element!bool(builder, status, "online");
        _frequency = add_element!Frequency(builder, status, "frequency");
        _fault = add_element!bool(builder, status, "fault");

        Component control = builder.component("control", "PowerControl");
        builder.constant(control, "kind", "continuous");
        builder.constant(control, "direction", "consume");
        builder.constant(control, "unit", "%");
        builder.constant(control, "min", 0.0f);
        builder.constant(control, "max", 100.0f);
        builder.constant(control, "can_disable", true);
        _level_e = add_element!float(builder, control, "level", Access.read_write);
        _mode_e = add_element!FireMode(builder, control, "mode", Access.read_write);
        _enable_e = add_element!bool(builder, control, "enable", Access.read_write);
        _applied = add_element!float(builder, control, "applied");
...
private:
    ...
    Element* add_element(T)(ref DeviceBuilder b, Component parent, const(char)[] id, Access access = Access.read,
