...
{
    alias Properties = AliasSeq!(Prop!("interface", iface), Prop!("address", address), Prop!("time", time),
...
private:
    ...
    static PCF85063Error decode_time(const(ubyte)[] data, out DateTime value)
    {
        ...
        uint second = from_bcd(data[0] & 0x7F);
        uint minute = from_bcd(data[1] & 0x7F);
        uint hour = from_bcd(data[2] & 0x3F);
        uint day = from_bcd(data[3] & 0x3F);
        uint weekday = data[4] & 0x07;
        uint month = from_bcd(data[5] & 0x1F);
        uint year = from_bcd(data[6]);
