...
{
    alias Properties = AliasSeq!(Prop!("device", device), Prop!("frequency", frequency), Prop!("sda-gpio", sda_gpio),
...
nothrow @nogc:
    ...
    {
        if (value.length == 4 && value[][0 .. 3] == "i2c")
        {
            uint port = value[][3] - '0';
            if (port < i2c_count())
            {
    ...
    static bool operation_complete(ref I2cOperation operation, I2cCallbackContext context)
    {
        ...
        final switch (context)
        {
            case I2cCallbackContext.task:
                queued = g_app.post_event(&instance.completion_event, getTime(), EventPriority.control);
                break;
            case I2cCallbackContext.interrupt:
                higher_priority_task_woken = g_app.post_event_from_isr(&instance.completion_event, EventPriority.control, queued);
                break;
        }
        if (!queued)
        {
