...
nothrow @nogc:
    ...
    final void detach_cpu_port()
    {
        _cpu.active = false;
        _cpu.send = null;
        _address_table.remove_port(_cpu_port, _cpu_port);
    }
...
protected:
    ...
    override CompletionStatus shutdown()
    {
        while (_tracking_active)
            abort(_tracking_active.bridge_tag);
...
private:
        ...
        void on_port_callback(int port_tag, MessageState state) nothrow @nogc
        {
            ...
            if (--pending == 0)
            {
                if (upstream_cb)
                {
                    auto cb = upstream_cb;
                    int tag = bridge_tag;
                    auto result = any_succeeded ? MessageState.complete : MessageState.failed;
                    bridge.unlink_tracking(&this);
                    bridge.recycle_tracking(&this);
                    cb(tag, result);
                }
    ...
    void unlink_tracking(TagTracking* entry)
    {
...
nothrow @nogc:
    ...
    void unsubscribe(ChangedHandler handler)
    {
        _subscribers.removeFirst(handler);
    }

    override bool disabled() const pure
        => super.disabled;

    override void disabled(bool value)
    {
...
private:
    Array!ChangedHandler _subscribers;
...
private:
    void ports_changed(BaseObject obj, CollectionEvent event)
    {
        ...
        {
            port.subscribe(&port_changed);
            port.reconcile();
        }
    }

    void port_changed(BridgePort port)
    {
        port.reconcile();
    }
...
unittest
{
    ...
    unpublished.subscribe(&observer.row_changed);
    assert(unpublished.pvid(43) is null);
    assert(observer.row_calls == 1);
    auto comment = "no membership change".make_string();
    unpublished.comment = comment;
    assert(observer.row_calls == 1);
    unpublished.disabled = true;
    assert(observer.row_calls == 2);
    assert(observer.first_calls == 1 && observer.third_calls == 0);
    ...
    auto link_downs = member.link_downs;
    ...
    assert(member.rx_packets == 1 && member.rx_dropped == 1);
    ...
    member.receive(packet);
    assert(member.rx_packets == 2 && member.rx_dropped == 1);
    master.disabled = true;
    assert(member.running && (member.flags & ObjectFlags.slave));
    member.receive(packet);
    assert(member.rx_packets == 3 && member.rx_dropped == 2);
    master.destroy();
    assert(Collection!BridgePort().get(port.name[]) is port);
    member.receive(packet);
    assert(member.rx_packets == 4 && member.rx_dropped == 3);
    master = Collection!BridgeInterface().create("bridge-test-master");
    assert(master.running && member.running && master.member_count == 1);
    member.receive(packet);
    assert(member.rx_packets == 5 && member.rx_dropped == 3);
    assert(member.link_downs == link_downs);

    struct PacketObserver
    {
        BridgeInterface master;
        uint calls;
        void received(ref const Packet, BaseInterface, PacketDirection, void*) nothrow @nogc
        {
            ++calls;
            master.disabled = true;
        }
    ...
    packet_observer.master = master;
    PacketFilter filter;
    member.subscribe(&packet_observer.received, filter);
    member.receive(packet);
    assert(packet_observer.calls == 1 && member.running && !master.running);
    assert(member.rx_packets == 6 && member.rx_dropped == 4);
    member.receive(packet);
    assert(packet_observer.calls == 1 && member.rx_packets == 7 && member.rx_dropped == 5);
    member.unsubscribe(&packet_observer.received);
    ...
    assert(!(member.flags & ObjectFlags.slave));
    master = Collection!BridgeInterface().create("bridge-test-offload");
    port = Collection!BridgePort().alloc("bridge-test-offload-port");
    assert(port.bridge(master.name[]) is null);
    assert(port.interface_name(member.name[]) is null);
    Collection!BridgePort().add(port);
    member.start();
    packet.eth.src = MACAddress(2, 0, 0, 0, 0, 9);
    packet.eth.dst = master.mac;
    master._cpu.active = true;
    master.cpu_port_incoming(packet);
    ulong learned = master.source_address(packet);
    assert(master._address_table.get(learned) == master._cpu_port);
    master.detach_cpu_port();
    assert(master._address_table.get(learned) == -1);
    packet.eth.dst = packet.eth.src;
    packet.eth.src = master.mac;
    uint before = member.sends;
    assert(master.transmit(packet, null, null) == 0);
    assert(member.sends == before + 1);
    port.destroy();
    master.destroy();
    ...
    struct ReentrantCompletion
    {
        ...
        uint calls;
        bool destroy_master;
        void completed(int, MessageState) nothrow @nogc
        {
            ++calls;
            if (destroy_master)
                master.destroy();
    ...
    foreach (mode; 0 .. 8)
    {
        master = Collection!BridgeInterface().create("bridge-test-reentrant");
        port = Collection!BridgePort().alloc("bridge-test-reentrant-port");
        assert(port.bridge(master.name[]) is null);
        assert(port.interface_name(member.name[]) is null);
        Collection!BridgePort().add(port);
        member.start();
        TestPort second;
        BridgePort second_port;
        if (mode >= 4)
        {
            second = Collection!TestPort().create("bridge-test-second");
            second_port = Collection!BridgePort().alloc("bridge-test-second-port");
            assert(second_port.bridge(master.name[]) is null);
            assert(second_port.interface_name(second.name[]) is null);
            Collection!BridgePort().add(second_port);
            second.start();
        }
        ReentrantCompletion reentrant;
        reentrant.port = port;
        reentrant.master = master;
        reentrant.destroy_master = mode % 4 == 1;
        packet.eth.dst = MACAddress(0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
        tag = master.transmit(packet, &reentrant.completed, null);
        assert(tag > 0);
        if (mode % 4 < 2)
        {
            auto complete = member.pending;
            member.pending = null;
            complete(17, MessageState.complete);
            if (second)
            {
                assert(reentrant.calls == 0);
                complete = second.pending;
                second.pending = null;
                complete(17, MessageState.complete);
            }
        }
        ...
        {
            master.disabled = true;
            assert(master.shutdown() == CompletionStatus.complete);
        }
        else
            master.abort(tag);
        assert(reentrant.calls == 1 && master._tracking_active is null);
        uint free_count;
        for (auto entry = master._tracking_free; entry; entry = entry.next)
            assert(++free_count <= master._tracking_batch_size);
        assert(free_count == master._tracking_batch_size);
        if (Collection!BridgePort().get("bridge-test-reentrant-port"))
            port.destroy();
        if (Collection!BridgeInterface().get("bridge-test-reentrant"))
            master.destroy();
        if (second_port)
        {
            second_port.destroy();
