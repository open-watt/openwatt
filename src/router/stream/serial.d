...
nothrow @nogc:
    ...
    version (Espressif)
    {
        static bool uart_rx_ready(Uart uart, size_t, UartCallbackContext context)
        {
            ...
            {
                bool queued;
                bool wake = g_app.post_event_from_isr(&instance.uart_rx_event, EventPriority.bulk, queued);
                if (!queued)
                {
            ...
            if (!g_app.post_event(&instance.uart_rx_event, getTime(), EventPriority.bulk))
            {
                atomicStore!(MemoryOrder.release)(instance._rx_event_pending, 0u);
                atomicStore!(MemoryOrder.release)(instance._rx_event_retry, 1u);
            }
