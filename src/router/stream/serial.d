...
nothrow @nogc:
    ...
    final uint baud_rate() const
        => prop_value!uint(prop_index!(SerialStream, "baud-rate"));
    final void baud_rate(uint value)
        => prop_write(prop_index!(SerialStream, "baud-rate"), value);

    final ubyte data_bits() const
        => prop_value!ubyte(prop_index!(SerialStream, "data-bits"));
    final void data_bits(ubyte value)
        => prop_write(prop_index!(SerialStream, "data-bits"), value);

    final Parity parity() const
        => prop_value!Parity(prop_index!(SerialStream, "parity"));
    final void parity(Parity value)
        => prop_write(prop_index!(SerialStream, "parity"), value);

    final StopBits stop_bits() const
        => prop_value!StopBits(prop_index!(SerialStream, "stop-bits"));
    final void stop_bits(StopBits value)
        => prop_write(prop_index!(SerialStream, "stop-bits"), value);

    final FlowControl flow_control() const
        => prop_value!FlowControl(prop_index!(SerialStream, "flow-control"));
    final void flow_control(FlowControl value)
        => prop_write(prop_index!(SerialStream, "flow-control"), value);

    version (Embedded)
    {
        final byte tx_gpio() const
            => prop_value!byte(prop_index!(SerialStream, "tx-gpio"));
        final byte rx_gpio() const
            => prop_value!byte(prop_index!(SerialStream, "rx-gpio"));
        final byte rts_gpio() const
            => prop_value!byte(prop_index!(SerialStream, "rts-gpio"));
        final byte cts_gpio() const
            => prop_value!byte(prop_index!(SerialStream, "cts-gpio"));
        final byte de_gpio() const
            => prop_value!byte(prop_index!(SerialStream, "de-gpio"));
