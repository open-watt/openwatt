...
//
// Lifetime: created once at EnergyModule init, persists for the life of the
// process. Vehicle Components are added on first vehicle_for(vin) call and
// kept across session connect/disconnect (last-known values persist with stale
// timestamps).
__gshared Device g_vehicles_device;


// Create the singleton vehicles Device. Called once from EnergyModule.init.
...
// Find an existing Vehicle Component by VIN, or create one with all standard
// sub-components stubbed (info, status, battery, meter, control). Manufacturer
// and model are derived from the VIN automatically.
Component vehicle_for(const(char)[] vin)
{
    ...
    vehicle.find_or_create_element("meter.type").value = StringLit!"ac";
