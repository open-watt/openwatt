...
// NHTSA vPIC: free public VIN decoder run by the US DOT. Returns ~150 fields including
...
//
// Fire-and-forget, called once per VIN from vehicle_for(); the find-existing-component
// check there is the dedup. Failure is silent, leaving the locally-decoded values.
...
// Battery capacity estimator: persistent running mean per VIN; each charging session feeds (delta_energy,
...
{
    float sum_weighted = 0;   // sum(estimate_kwh * weight)
    float weight_total = 0;   // sum(weight)
    uint  sample_count = 0;
