...
import apps.energy.model : BusType, MeterSign, is_multi_phase;
...
bool is_active_energy_field(MeterField field) pure
{
    switch (field)
    {
        case MeterField.total_active:
        case MeterField.total_import_active:
        case MeterField.total_export_active:
        case MeterField.total_absolute_active:
            return true;
        default:
            return false;
    }
...
enum Provenance : ubyte
{
    ...
    inferred_sum,           // sum of adjacent values (unmetered bus)
    inferred_subtraction,   // parent minus other known children
    rogue,                  // unaccounted remainder at a metered bus/link
}

const(char)[] provenance_name(Provenance p) pure nothrow @nogc
{
    final switch (p)
    {
        case Provenance.missing:              return "missing";
        case Provenance.measured:             return "measured";
        case Provenance.synthesized:          return "synthesized";
        case Provenance.quadrant_derived:     return "quadrant-derived";
        case Provenance.inferred_sum:         return "inferred-sum";
        case Provenance.inferred_subtraction: return "inferred-subtraction";
        case Provenance.rogue:                return "rogue";
    }
}
...
nothrow @nogc:
    ...
    // Provenance is authoritative; fields is only a cheap summary.
    ...
    // Topology inference iterates fields generically.
...
static BusType get_meter_type(Component meter)
{
    ...
    switch(e && e.value.isString ? e.value.asString : "")
    {
        ...
        case "split-phase":     return BusType.split_phase;
        case "three-phase":     return BusType.three_phase;
        case "delta":           return BusType.delta;
        default:                break;
    }
    return BusType.unknown;
}

MeterData extract_phase(ref const MeterData raw, BusType type, ubyte meter_phase) pure
{
    MeterData r;
    r.reset_to_missing();
    r.type = type;
...
MeterData get_port_meter_data(Component meter, ubyte meter_phase, MeterSign sign, FieldFlags fields = FieldFlags.all)
{
    MeterData raw = get_meter_data(meter, fields);
    BusType type = meter_phase != 0 ? BusType.single_phase : raw.type;
    MeterData data = extract_phase(raw, type, meter_phase);
    apply_meter_sign(data, sign);
    return data;
}

void apply_meter_sign(ref MeterData data, MeterSign sign) pure
{
    if (sign == MeterSign.normal)
        return;

    negate_field(data, MeterField.current);
    negate_field(data, MeterField.power);
    negate_field(data, MeterField.reactive);
    negate_field(data, MeterField.phase_angle);
    negate_field(data, MeterField.total_active);
    negate_field(data, MeterField.total_reactive);
    swap_fields(data, MeterField.total_import_active, MeterField.total_export_active);
    swap_fields(data, MeterField.total_reactive_import, MeterField.total_reactive_export);
    swap_fields(data, MeterField.total_apparent_import, MeterField.total_apparent_export);
    swap_fields(data, MeterField.total_inductive, MeterField.total_capacitive);
    rebuild_field_flags(data);
}
...
unittest
{
    ...
    data.write_value(MeterField.power, 0, 123.0f);
    data.mark(MeterField.power, 0, Provenance.measured);
    data.write_value(MeterField.total_active, 0, 10.0f);
    data.mark(MeterField.total_active, 0, Provenance.measured);
    data.write_value(MeterField.total_import_active, 0, 10.0f);
    data.mark(MeterField.total_import_active, 0, Provenance.measured);

    apply_meter_sign(data, MeterSign.inverted);

    assert(data.active[0].value == -123.0f);
    assert(data.total_active[0] == -10.0f);
    assert(!data.has(MeterField.total_import_active));
    assert(data.has(MeterField.total_export_active));
    assert(data.total_export_active[0] == 10.0f);
    assert(!(data.fields & FieldFlags.total_import_active));
    assert(data.fields & FieldFlags.total_export_active);
}
...
static MeterData get_meter_data(Component meter, FieldFlags fields = FieldFlags.all)
{
    ...
    // Track element-read provenance locally while synthesis mutates r.
    ...
    for (size_t i = 0, bit = 1; i < num_fields; ++i, bit <<= 1)
    {
        ...
        for (size_t j = 0; j < 4; ++j)
        {
            ...
            {
                double v = is_active_energy_field(cast(MeterField)i)
                         ? e.scaled_value!KilowattHour()
                         : e.normalised_value();
                if (v == v)
                {
                    f[i][j] = v;
        ...
        if ((values_present & 3) == 1 && r.type == BusType.single_phase)
        {
            f[i][1] = f[i][0];
            present[i] |= 2;
    ...
    if (r.type == BusType.dc)
    {
        if ((r.fields & primary_fields) == (FieldFlags.voltage | FieldFlags.current))
        {
            foreach (i; 0..4)
            {
                r.active[i] = r.voltage[i] * r.current[i];
        ...
        else if ((r.fields & primary_fields) == (FieldFlags.voltage | FieldFlags.power))
        {
            foreach (i; 0..4)
            {
                if (r.voltage[i] == MeterVolts(0))
                    ...
                    r.current[i] = r.active[i] / r.voltage[i];
        ...
        else if ((r.fields & primary_fields) == (FieldFlags.current | FieldFlags.power))
        {
            foreach (i; 0..4)
            {
                if (r.current[i] == MeterAmps(0))
                    ...
                    r.voltage[i] = MeterVolts(r.active[i] / r.current[i]);
    ...
    else
    {
        ...
        {
            // S must exist before the P/Q/S triangle can complete sparse meters.
            foreach (i; 0..4)
            {
        ...
        else if ((r.fields & primary_fields) == (FieldFlags.voltage | FieldFlags.power))
        {
            ...
            foreach (i; 0..4)
            {
                if (r.voltage[i] == MeterVolts(0))
                    ...
                    r.current[i].value = r.active[i].value / r.voltage[i].value; // assumes unity PF
                if (r.has(MeterField.voltage, cast(ubyte)i) && r.has(MeterField.power, cast(ubyte)i))
                    r.mark(MeterField.current, cast(ubyte)i, Provenance.synthesized);
            }
        }
    ...
    // Reactive sign distinguishes inductive and capacitive phase angles.
...
void negate_field(ref MeterData data, MeterField field) pure
{
    foreach (phase; 0..4)
        if (data.has(field, cast(ubyte)phase))
            data.write_value(field, cast(ubyte)phase, -data.read_value(field, cast(ubyte)phase));
}

void swap_fields(ref MeterData data, MeterField a, MeterField b) pure
{
    foreach (phase; 0..4)
    {
        float av = data.read_value(a, cast(ubyte)phase);
        float bv = data.read_value(b, cast(ubyte)phase);
        Provenance ap = data.provenance[a][phase];
        Provenance bp = data.provenance[b][phase];
        data.write_value(a, cast(ubyte)phase, bv);
        data.write_value(b, cast(ubyte)phase, av);
        data.provenance[a][phase] = bp;
        data.provenance[b][phase] = ap;
    }
}

void rebuild_field_flags(ref MeterData data) pure
{
    data.fields = FieldFlags.none;
    foreach (i; 0..num_fields)
        foreach (phase; 0..4)
            if (data.provenance[i][phase] != Provenance.missing)
            {
                data.fields |= cast(FieldFlags)(1 << i);
                break;
            }
}
