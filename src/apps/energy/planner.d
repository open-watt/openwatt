...
import urt.si.unit : ScaledUnit, AmpereHour, Percent, Volt;
import urt.string;
import urt.time;
...
nothrow @nogc:
              ...
              ref TopologyGraph graph, SysTime now)
    {
        ...
        // Compute budgets before their pressure modifier is applied to policies.
        ...
        if (cap == cap && cap > 0)
        {
            ...
            if (Element* soc_e = c.find_element("soc"))
                if (soc_e.value.isNumber)
                    soc = read_in_unit(soc_e, Percent);
            if (soc == soc)
...
float battery_capacity_kwh(Component target)
{
    ...
    if (cap_e is null || !cap_e.value.isNumber)
        return float.nan;
    float ah = read_in_unit(cap_e, AmpereHour);
    ...
    if (v_e is null || !v_e.value.isNumber)
        return float.nan;
    float voltage = read_in_unit(v_e, ScaledUnit(Volt));
    if (voltage <= 0)
        return float.nan;
