...
// analysis to energy.policy.<name>.planner.* for inspection. NOT YET wired
// back into the allocator's policy ordering — that's a separate review step.
struct Planner
{
nothrow @nogc:
    ...
    void tick(Device energy_device, ControlRegistry registry, ref Archipelago archipelago, SysTime now)
    {
        ...
        {
            IslandBudget b = compute_island_budget(island, registry, now, slack_threshold, reserve_safety_factor);
        ...
        // Per-policy analysis (with pressure mod from refreshed budgets).
        foreach (Policy p; Collection!Policy().values)
        {
            IslandBudget* b = budget_for_policy(p, archipelago);
            PolicyAnalysis a = analyse_policy(p, registry, now, slack_threshold, b);
            publish_analysis(energy_device, p, a, now);
        }
    }

    IslandBudget* budget_for_policy(Policy p, ref Archipelago archipelago)
    {
        Island* island = find_policy_island_in(p, archipelago);
        if (island is null)
            return null;
...
IslandBudget compute_island_budget(Island* island, ControlRegistry registry, SysTime now, Duration slack_threshold, float safety_factor)
{
    IslandBudget b;
    if (island is null)
        return b;
    ...
    foreach (Policy p; Collection!Policy().values)
    {
        ...
        if (pi !is island)
            continue;
        PolicyAnalysis pa = analyse_policy(p, registry, now, slack_threshold);
...
PolicyAnalysis analyse_policy(Policy p, ControlRegistry registry, SysTime now, Duration slack_threshold, const(IslandBudget)* budget = null)
{
    PolicyAnalysis a;
    ...
    a.max_rate_kw = (ctl !is null) ? control_max_rate_kw(*ctl) : float.nan;
    a.required_kwh = required_energy_kwh(p, ctl);
...
Island* find_policy_island(Policy p, Island* candidate)
{
    ...
    if (target is null)
        return null;
    // Walk up the circuit chain from the appliance's circuit; if any ancestor
    // is a member of `candidate`, this is the policy's island.
    Circuit* c = target.circuit_ref;
    while (c !is null)
    {
        foreach (m; candidate.members[])
            if (m is c)
                return candidate;
        c = c.parent;
    }
...
Island* find_policy_island_in(Policy p, ref Archipelago archipelago)
{
    foreach (island; archipelago[])
...
void publish_island_budget(Device energy_device, Island* island, ref const IslandBudget b, SysTime now)
{
    if (energy_device is null || island is null)
        return;
    const(char)[] base = tconcat("archipelago.island.", island.id[], ".budget");
...
float required_energy_kwh(Policy p, const Control* ctl)
{
    final switch (p.parsed_goal.kind) with (GoalKind)
    {
        ...
        case soc:
            ...
            if (current != current || current >= goal)
                return 0;
            float capacity_kwh = appliance_battery_capacity_kwh(p.target_appliance);
        ...
        case duty:
            ...
            if (ctl is null)
                return float.nan;
            float rate_kw = control_max_rate_kw(*ctl);
            if (rate_kw != rate_kw)
                return float.nan;
...
float control_max_rate_kw(ref const Control ctl)
{
    ...
    final switch (ctl.unit) with (ControlUnit)
    {
        ...
        case A:
            // HACK: assumes 230V nominal mains. Pull from the target's island root meter voltage once that lookup is wired.
            float mx = ctl.max;
            return (mx == mx && mx > 0) ? mx * 230.0f * 0.001f : float.nan;
...
float appliance_battery_capacity_kwh(Appliance a)
{
    if (a is null)
        return float.nan;
    // Look in the primary device first, then paired partner (for car composites
    // where the SOC and capacity come from the partner's BLE surface).
    Component[2] roots = [a.device_ref, a.paired_with ? a.paired_with.device_ref : null];
    foreach (root; roots)
    {
        float c = battery_capacity_kwh(root);
        if (c == c)
            return c;
    }
    return float.nan;
