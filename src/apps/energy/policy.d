...
protected:
    override bool validate() const
    {
        ...
        // here — the appliance's Control may not yet exist (placeholder/unpaired
...
// Pick the element on a Control that represents the current state for a given
// goal kind. Returns null if the Control lacks a usable witness.
...
bool satisfied(Policy p, const(Control)* ctl)
{
    ...
    final switch (p.parsed_goal.kind) with (GoalKind)
    {
        ...
        case expression:
            ...
            // appliance-level state (e.g. summed power, paired SOC) rather than
            // a single Component.
