...
nothrow @nogc:
    ...
    // Composite graph controls may later add a separate enable_e to bridge split actuators.
...
nothrow @nogc:
    ...
    // Graph composites let a policy target a car while the setpoint lives on its EVSE.
    ...
    // Cheap at expected scale (tens of appliances). Preserves allocator-owned
    // runtime state (last_transition, current_setpoint) across rebuilds.
    //
    ...
    void resync_all(ref TopologyGraph graph)
    {
        ...
        foreach (Appliance a; col.values)
        {
            ...
            // Composite controls will be synthesized from graph adjacency in a later pass.
            Control synth;
