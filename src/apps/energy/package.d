...
nothrow @nogc:
    ...
    void add_control_row(ref Table table, ref Control ctl, char[] path_buf)
    {
        ...
        if (ctl.setpoint_e is null)
            table.cell("-");
        else if (ctl.setpoint_e.value.isBool)
            table.cell(ctl.setpoint_e.value.asBool ? "on" : "off");
        else if (!ctl.setpoint_e.value.isNumber)
            table.cell("-");
        else
        {
            final switch (ctl.unit) with (ControlUnit)
            {
                case A:                  table.cell(tconcat(cast(Amps)ctl.setpoint_e.value.asQuantity)); break;
                case W:                  table.cell(tconcat(cast(Watts)ctl.setpoint_e.value.asQuantity)); break;
                case percent:            table.cell(tconcat(ctl.setpoint_e.value.asFloat, "%")); break;
                case boolean:
                case nameplate_fraction:
                case unknown:            table.cell(tconcat(ctl.setpoint_e.value.asFloat)); break;
            }
        }
    ...
    // A recursive prefix must outlive churn in urt's temporary ring.
    static struct TreePrefix
    {
    nothrow @nogc:
        const(char)[] text() const return pure => _buf[0 .. _len];

        TreePrefix opBinary(string op : "~")(const(char)[] segment) const pure
        {
            TreePrefix r = this;
            size_t n = segment.length;
            if (n > _buf.length - _len)
                n = _buf.length - _len;
            r._buf[_len .. _len + n] = segment[0 .. n];
            r._len = cast(ubyte)(_len + n);
            return r;
        }
    ...
    Table build_circuit_table(Session)
    {
                          ...
                          ref Array!(Bus*) visited, bool emit_self = true)
        {
            ...
            {
                if (ingress)
                    add_link_row(ingress, prefix.text, connector);
                else
            ...
            foreach (a; Collection!Appliance().values)
            {
                ...
                add_appliance_row(a, child_prefix.text, last);

                size_t child_total = appliance_child_count(a, bus);
                if (child_total == 0)
                    continue;

                TreePrefix appliance_prefix = child_prefix ~ (last ? "   " : "│  ");
                ...
                foreach (link; this.manager.graph.links[])
                {
                    ...
                    TreePrefix lane_prefix = child_last ? appliance_prefix : appliance_prefix ~ "│  ";
            ...
            {
                ++emitted;
                add_implicit_member_row(ingress, child_prefix.text, emitted == total);
            }

            if (has_unaccounted)
            {
                ++emitted;
                add_unaccounted_row(bus, child_prefix.text, emitted == total);
            }
        }
        ...
        bool add_grid_root(Bus* bus, ref Array!(Bus*) visited)
        {
            ...
            foreach (link; bus.links[])
            {
                ...
                bool last = emitted == total;
                if (link.closed && visited[].findFirst(link.b) >= visited.length)
                    add_bus_tree(link.b, link, TreePrefix(), last ? "└─ " : "├─ ", visited);
                else
        ...
        foreach (root; roots[])
        {
            ...
            if (root.contains_grid && add_grid_root(root, visited))
                continue;
            add_bus_tree(root, null, TreePrefix(), "", visited);
        }
        ...
        foreach (bus; this.manager.graph.bus_list[])
        {
            ...
            add_bus_tree(bus, null, TreePrefix(), "", visited);
        }
...
nothrow @nogc:
    ...
    override void render_content(uint offset, uint count, uint width)
    {
        ...
        build_table().render_viewport(session, offset, count, _sticky_widths[]);
    }

protected:
    abstract Table build_table();
...
protected:
    override Table build_table()
        => _mod.build_island_table();
}
...
protected:
    override Table build_table()
        => _mod.build_topology_table(session);
}
...
nothrow @nogc:
    ...
    override uint content_height()
        => _mod.build_circuit_table(session).num_rows;

    override const(char)[] status_text()
    {
        import urt.mem.temp : tconcat;
        return tconcat(_mod.build_circuit_table(session).num_rows, " circuit rows");
    }

protected:
    override Table build_table()
        => _mod.build_circuit_table(session);
}
unittest
{
    import urt.mem.temp : tconcat;

    alias TreePrefix = EnergyAppModule.TreePrefix;
    ...
    // Prefix storage must survive temporary-ring wraparound.
    foreach (i; 0 .. 4096)
        cast(void)tconcat("row ", i, " padding padding padding");
    assert(a.text == "|  ");
    assert(b.text == "|     ");
