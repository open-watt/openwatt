    ...
    final switch (fmt.type) with (ValueType)
    {
        ...
        case u8, s8, u16, s16, u32, s32, u64, s64:
        {
            ...
            else
            {
                ...
                {
                    if (v % base != 0)
                        return false; // a fractional value can't land in an integer record
                    v /= base;
                }
            }
...
{
    if (buf.length < wire.length)
        return null; // a partial field would leave the swizzle a non-word multiple
