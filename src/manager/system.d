...
version (Tiny) {} else
void page_pool(Session session)
{
    ...
    {
        session.write_line(label, ": in-use ", s.pages_in_use, ", free ", s.pages_free, ", slabs ", s.slab_count, ", peak ", s.high_water, ", allocs ", s.alloc_count, ", fails ", s.fail_count);
    ...
    PagePoolStats heap = page_pool_stats(page_category_heap);
    if (heap.alloc_count != 0 || heap.fail_count != 0)
        write_stats(session, "heap pages", heap);
}
