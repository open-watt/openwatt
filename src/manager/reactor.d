...
// overlapped read parked per watch).
//
...
nothrow @nogc:
    ...
    void set()
    {
        ...
        {
            if (cas(&_signalled, false, true) && _waiter !is null)
    ...
    // Sweeps ready I/O before honouring the latch, so I/O never starves behind a wake;
    // dispatched I/O counts as a wake so the loop services any follow-up promptly.
...
private:
    ...
    version (Windows)
    {
        ...
        {
        nothrow @nogc:
            IoOp op;            // must be first: completion looks up the entry through it
            Reactor* owner;
            HANDLE file;
        ...
        bool dispatch(epoll_event* evs, int n)
        {
            ...
            foreach (i; 0 .. n)
            {
                ...
                if (ptr is &_pool_tag)
                {
                    ...
                    // dead fd from their collect (or restart).
                    did_io = true;
                    _pool_pending = true;
                    continue;
                }
